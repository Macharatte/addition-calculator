...
# --- 1. システム状態管理 ---
if 'v13_full_restore' not in st.session_state:
    st.session_state.clear()
    st.session_state.v13_full_restore = True
...
L_MAP = {
    ...
    "韓国어": {"upd": "갱신", "thm": "테마", "clr": "삭제", "exe": "계산", "si": "접두어", "sci": "과학", "stat": "수치", "paid": "유료", "fuel": "가솔린", "cur": "환율", "tax": "세금", "mean":"평균", "sum":"합계", "mode":"최빈", "med":"중앙", "max":"최대", "min":"최소", "dev":"편차", "exp":"기대"},
...
<style>
    ...
    .disp {{
        ...
        font-family: monospace; margin-bottom: 20px; min-height: 90px; overflow-x: auto;
    }}
    div.stButton > button {{
    ...
    .paid-box {{ border: 3px solid {text_color} !important; padding: 20px; border-radius: 10px; }}
...
# --- 7. 有料機能 (燃料・通貨・税金) ---
with t_paid:
    mode = st.radio("MODE", [L["fuel"], L["cur"], L["tax"]], horizontal=True)
    ...
    if mode == L["fuel"]:
        ...
        lit = st.number_input("Litre", 1.0, 1000.0, 50.0, step=1.0)
        st.markdown(f"### **{int(lit * u_price):,} JPY** ({u_price}円/L)")

    elif mode == L["cur"]:
        cur_type = st.radio("TARGET", ["USD", "BTC", "ETH"], horizontal=True)
        amt = st.number_input("Amount", 0.0, 1000000.0, 1.0, step=0.1)
        rate = st.session_state.rates.get(cur_type, 1.0)
        st.markdown(f"### **{amt * rate:,.2f} JPY**")

    elif mode == L["tax"]:
        base_amt = st.number_input("Base Amount", 0.0, 10000000.0, 10000.0, step=100.0)
        tax_sel = st.radio("Tax Rate", ["8%", "10%", "Custom"], horizontal=True)
        if tax_sel == "Custom":
            tax_rate = st.slider("Rate (%)", 0, 50, 15) / 100
        else:
            tax_rate = 0.08 if tax_sel == "8%" else 0.10
        st.markdown(f"### TAX: **{int(base_amt * tax_rate):,} JPY**")
        st.markdown(f"### Total: **{int(base_amt * (1+tax_rate)):,} JPY**")
