...
# --- 3. 全言語リソース (モード名を番号で管理するための辞書) ---
LANG = {
    "JP": {"title": "Python Calculator Premium", "modes": ["通常", "科学計算", "値数", "拡縮", "有料機能"], "tax_m": "税金", "cur_m": "為替", "exec": "実行", "amt": "入力"},
    "EN": {"title": "Python Calculator Premium", "modes": ["Basic", "Scientific", "Stats", "SI Prefix", "Paid"], "tax_m": "Tax", "cur_m": "Currency", "exec": "Run", "amt": "Input"},
...
if 'm_idx' not in st.session_state: st.session_state.m_idx = 0  # 0:通常, 1:科学, 2:統計, 3:SI, 4:有料
...
# --- 9. モード別機能ロジック (完全に書き出し) ---
midx = st.session_state.m_idx

if midx == 1: # 科学計算
    sc = st.columns(4)
    s_keys = [("sin", "math.sin("), ("cos", "math.cos("), ("tan", "math.tan("), ("log", "math.log10("), 
              ("abs", "abs("), ("sqrt", "math.sqrt("), ("π", "math.pi"), ("e", "math.e")]
    ...
    sc = st.columns(4)
    st_keys = [("Mean", "statistics.mean(["), ("Median", "statistics.median(["), ("Mode", "statistics.mode(["), 
               ("Max", "max(["), ("Min", "min(["), (",", ","), ("]", "])")]
    for i, (lbl, code) in enumerate(st_keys):
        if sc[i % 4].button(lbl, key=f"st_{i}"):
            st.session_state.f_state += code; st.rerun()

elif midx == 3: # 拡縮 (SI接頭語 Q/R対応)
...
elif midx == 4: # 有料機能 (税金・為替)
    sub = st.columns(2)
    if sub[0].button(L["tax_m"], key="p_tax"): st.session_state.p_sub = "tax"; st.rerun()
    if sub[1].button(L["cur_m"], key="p_cur"): st.session_state.p_sub = "cur"; st.rerun()
    
    if st.session_state.p_sub == "tax":
        t_type = st.selectbox("Type", ["Income Tax (20%)", "Inheritance Tax", "Consumption 10%", "Consumption 8%"])
        amt = st.text_input(L["amt"], key="tax_in")
        if st.button(L["exec"], key="tax_run"):
            try:
                v = float(amt) if amt else 0.0
                res = v * 1.1 if "10%" in t_type else v * 1.08 if "8%" in t_type else v * 0.2
        ...
        c1, c2 = st.selectbox("From", c_list), st.selectbox("To", c_list)
        amt_c = st.text_input(L["amt"], key="cur_in")
        if st.button(L["exec"], key="cur_run"):
            try:
                # 簡易レート
                rates = {"JPY": 150.0, "USD": 1.0, "EUR": 0.92, "XAU": 0.0004}
                v = float(amt_c) / rates[c1.split()[0]]
                res = v * rates[c2.split()[0]]
                st.session_state.tax_res = f"Result: {format(res, '.4f')} {c2.split()[0]}"
            except: st.session_state.tax_res = "Error"
            st.rerun()
        st.markdown(f'<div class="tax-box">{st.session_state.tax_res}</div>', unsafe_allow_html=True)
