...
# --- ページ設定 ---
st.set_config(page_title="Python Calculator Pro 2", layout="centered")
...
for i, m in enumerate(m_list):
    if m_cols[i].button(m): ss.mode = m; ss.premium_sub = "なし"; st.rerun()
...
if ss.mode == "有料機能":
    ...
    if ss.premium_sub == "通貨":
        ...
        if st.button("変換実行"):
            ...
            ss.conv_result = f"{format(res, ',.2f')} {t_code}"
            st.rerun()
        if ss.conv_result:
            st.markdown(f'<div class="result-box">結果: {ss.conv_result}</div>', unsafe_allow_html=True)
    ...
    for i, b in enumerate(items):
        if e_cols[i % 5].button(b): ss.formula += b; st.rerun()
